...
router.post("/signup", async (req, res) => {
  ...
  if (!phone || !password || !firstName || !lastName)
    return res
      .status(400)
      .json({ error: "Phone, password, firstName and lastName are required" });
  ...
  async (req, res) => {
    ...
    try {
      ...
      let cartItem = user.cart.find(
        (ci) => ci.product && ci.product.equals(productId),
      );
      if (cartItem) {
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      let cartItem = user.cart.find(
        (ci) => ci.product && ci.product.toString() === productId.toString(),
      );
      if (!cartItem)
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (
        !user.wishlist.some((wi) => wi.product && wi.product.equals(productId))
      ) {
      ...
      res
        .status(500)
        .json({ message: "Error updating wishlist", error: err.message });
    }
  },
);
  ...
  async (req, res) => {
    ...
    try {
      ...
      user.wishlist = user.wishlist.filter(
        (wi) => wi.product && !wi.product.equals(productId),
      );
      await user.save();
      res.json({ wishlist: user.wishlist });
    } catch (err) {
      res
        .status(500)
        .json({ message: "Error removing wishlist item", error: err.message });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      const items = user.cart.map((ci) => ({
        ...
        total:
          ci.quantity * (ci.product.selling_Price?.price || ci.product.price),
      }));
