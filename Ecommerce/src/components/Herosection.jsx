...
const getColors = (accentColor) => {
  switch (accentColor) {
    case "blue":
      return { heading: "#1a237e", accent: "#1565c0", border: "#1976d2", text: "#222" };
    case "yellow":
      return { heading: "#b28704", accent: "#fbc02d", border: "#fbc02d", text: "#333" };
    case "green":
      return { heading: "#1b5e20", accent: "#388e3c", border: "#388e3c", text: "#222" };
    case "purple":
      return { heading: "#4a148c", accent: "#8e24aa", border: "#8e24aa", text: "#222" };
    case "red":
      return { heading: "#b71c1c", accent: "#d32f2f", border: "#d32f2f", text: "#222" };
    case "gray":
      return { heading: "#424242", accent: "#616161", border: "#757575", text: "#212121" };
    default:
      return { heading: "#b71c1c", accent: "#d32f2f", border: "#d32f2f", text: "#222" };
  }
};
...
const CategoryTiles = ({ categories }) => (
  <section className="max-w-6xl mx-auto px-4 pb-6">
    <div className="flex items-center justify-between mb-4">
      <h2 className="text-2xl font-extrabold text-[#1976d2]">Shop by Category</h2>
      <button
...
const ContactSection = ({ id = "contact" }) => (
  <section id={id} className="max-w-6xl mx-auto px-4 py-16 text-black rounded">
    <h2 className="text-2xl font-extrabold mb-8 border-l-4 pl-4" style={{ color: "#4a148c", borderColor: "#8e24aa" }}>
      Contact Us
    </h2>
...
const HeroBanner = ({ stats }) => (
  ...
  >
    ...
    <div className="flex-1 bg-white/70 rounded-2xl shadow p-6 border border-blue-100">
      <div className="text-sm uppercase tracking-[0.3em] text-[#1976d2] font-semibold mb-3">Discover</div>
      <h3 className="text-2xl font-extrabold text-[#0f172a] mb-3">Browse curated categories</h3>
      <p className="text-sm text-gray-600 mb-6">
        Explore everything in one place. Click any category to view items and details.
      </p>
      <div className="flex flex-wrap gap-2">
        {["Perishables", "Snacks", "Beverages", "Grains", "Bakery", "Dairy", "Offers"].map((label) => (
          <span key={label} className="px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-semibold">
            {label}
          </span>
        ))}
      </div>
...
export default function HeroSection() {
  ...
  const stats = useMemo(() => {
    const entries = Object.entries(allProducts || {}).filter(([, items]) => Array.isArray(items));
    const totals = entries.map(([category, items]) => ({ category, count: items.length }));
    const totalProducts = totals.reduce((sum, item) => sum + item.count, 0);
    const activeCategories = totals.filter((item) => item.count > 0).length;
    const topCategory = totals.sort((a, b) => b.count - a.count)[0]?.category || null;
