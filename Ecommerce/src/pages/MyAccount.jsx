  ...
  return (
    <div className="fixed z-50 inset-0 bg-black/50 flex items-center justify-center px-2">
      <div className="relative bg-gradient-to-br from-[#fff8ee] to-[#f9f9f9] rounded-2xl shadow-2xl p-8 w-full max-w-md border border-yellow-300 ring-1 ring-yellow-200 animate-fadeIn">
        <button
          className="absolute top-2 right-4 text-3xl text-[#d4af37] hover:text-[#222] transition"
          onClick={onClose}
        >
          &times;
        </button>
        <h2 className="text-xl font-bold text-center text-[#d4af37] mb-5 tracking-wide">
          Change Password
        </h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <label className="block">
            <span className="block mb-1 text-sm font-medium text-[#7f8c8d]">
              Old Password
            </span>
            <input
              type="password"
              required
              className="input block w-full rounded-lg border border-[#ccc] px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#d4af37] bg-[#faf6ee]"
              value={oldPassword}
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-full py-2 font-semibold text-white bg-gradient-to-r from-[#d4af37] to-[#a67c52] shadow-lg transition hover:bg-gradient-to-tr hover:from-[#bfa544] hover:to-[#9f7d4c] disabled:opacity-70"
            >
            ...
            <button
              type="button"
              onClick={onClose}
              className="w-full rounded-full py-2 font-semibold text-[#2c3e50] border border-[#d4af37] bg-[#faf6ee] shadow hover:bg-[#f2e8d8] transition"
            >
              Cancel
            </button>
          </div>
...
function PurchaseCard({ purchase }) {
  ...
  return (
    <div className="rounded-xl border border-[#e8e1d1] bg-gradient-to-r from-[#fff8ee] to-[#fcfaf7] shadow-sm overflow-hidden mb-6 transition">
      {/* Header */}
      <button
        className="w-full text-left flex justify-between items-center px-6 py-4 hover:bg-yellow-50 transition group"
      ...
      >
        ...
        <span
          className={`text-xl text-[#d4af37] transition duration-200 ${open ? "rotate-90" : ""}`}
        >
          ▶
        </span>
      </button>
      <div
        className={`${open ? "max-h-[800px] opacity-100" : "max-h-0 opacity-0"} transition-all duration-400 overflow-x-auto`}
      >
          ...
          <table className="w-full mt-2 rounded-md overflow-hidden bg-white shadow-sm text-lg">
            ...
            <tbody>
              {purchase.items.map((item, idx) => (
                <tr
                  key={item.name + idx}
                  className="odd:bg-[#f9f6ef]/50 even:bg-white border-b last:border-0"
                >
                  <td className="py-2 px-3 font-semibold text-[#273042]">
                    {item.name}
                  </td>
                  <td className="py-2 px-3 text-center">{item.quantity}</td>
                  <td className="py-2 px-3 text-right text-green-700 font-semibold">
                    ₹{toNumber(item.advancePaid).toLocaleString()}
                  </td>
                  <td className="py-2 px-3 text-right font-semibold">
                    ₹{toNumber(item.totalPrice).toLocaleString()}
                  </td>
                </tr>
              ))}
            </tbody>
...
function Account() {
  ...
  useEffect(() => {
    ...
    async function fetchUserData() {
      ...
      try {
        ...
        const wishlistData = wishlistRes.ok
          ? await wishlistRes.json()
          : { wishlist: [] };
        setWishlist(
          (wishlistData.wishlist || []).filter((item) => item?.product?._id),
        );
  ...
  const purchaseGraphData = (() => {
    ...
    userData.purchased_history.forEach((ph) => {
      ...
      byMonth[key] =
        (byMonth[key] || 0) +
        ph.items.reduce((sum, item) => sum + toNumber(item.totalPrice), 0);
    });
  ...
  return (
    <div className="max-w-[1400px] mx-auto min-h-screen px-4 md:px-12 py-10 md:py-20 font-montserrat text-[#333] bg-gradient-to-br from-[#fff8ee] to-[#f9f9f9]">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-10">
        <h1 className="text-3xl md:text-4xl font-extrabold text-yellow-700 uppercase tracking-wide">
          My Dashboard
        </h1>
        <div className="flex gap-3 mt-4 md:mt-0">
          <button
            className="px-6 py-3 rounded-full font-bold bg-gradient-to-r from-[#d4af37] to-[#a67c52] text-white shadow-lg transition hover:-translate-y-1 hover:shadow-lg"
            onClick={() => setChangeOpen(true)}
          >
            <Icon icon="mdi:lock" className="inline-block mr-2" /> Change
            Password
          </button>
          <button
            className="px-6 py-3 rounded-full font-bold bg-gradient-to-r from-[#e74c3c] to-[#c0392b] text-white shadow-lg transition hover:-translate-y-1 hover:shadow-lg"
            onClick={handleLogout}
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        ...
        <Link
          to="/wishlist"
          className="bg-pink-50 rounded-xl shadow flex flex-col items-center py-8 px-4 border-b-4 border-pink-300 hover:shadow-lg hover:-translate-y-1 transition"
        >
        ...
        <Link
          to="/cart"
          className="bg-green-50 rounded-xl shadow flex flex-col items-center py-8 px-4 border-b-4 border-green-300 hover:shadow-lg hover:-translate-y-1 transition"
        >
      ...
      <section className="bg-white rounded-2xl shadow p-8 mb-12 border border-yellow-100 grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col items-center justify-center">
          <Icon
            ...
            className="text-[7rem] text-yellow-200"
          />
        </div>
      </section>
      ...
      <section className="bg-pink-50 rounded-2xl shadow p-8 mb-12 border border-pink-100">
        ...
        {wishlist.length === 0 ? (
          <div className="text-gray-500">
            ...
            <Link className="text-pink-600 underline" to="/categories">
              Browse Products
            </Link>
          </div>
        ) : (
      ...
      <section className="bg-green-50 rounded-2xl shadow p-8 mb-12 border border-green-100">
        ...
        {cart.length === 0 ? (
          <div className="text-gray-500">
            Your cart is empty.{" "}
            <Link className="text-green-600 underline" to="/categories">
              Go Shopping
            </Link>
          </div>
        ) : (
            ...
            {cart.slice(0, 4).map((item, idx) => (
              <li key={idx} className="py-3 flex items-center gap-4">
                ...
                <span className="text-gray-600">
                  ...
                  {(
                    item.product?.price ||
                    item.product?.selling_Price?.price ||
                    0
                  ).toLocaleString()}
                </span>
              </li>
        ...
        <div className="mt-4 text-right">
          <Link to="/cart" className="text-green-600 underline font-semibold">
            View Full Cart
          </Link>
        </div>
      </section>
